...
export const AddMenuItems = ({item, idx, onChangeMenuItem, onRemoveMenuItem}) => {
    ...
    return (
        <div className="border rounded p-4 shadow mb-6">
            ...
            <div>
                {foodTags.map(tag => (
                    <button key={tag} type="button"  className={`mr-2 mb-2 px-2 py-1 border rounded ${item.foodTags.includes(tag) ? 'bg-green-500 text-white' : 'bg-gray-200 text-gray-700'}`} onClick={() => tagToggler(tag)}>{tag} {item.foodTags.includes(tag) && <span>&times;</span>}</button>
                ))}
            </div>
