...
export const AddMenuItems = ({item, idx, onChangeMenuItem, onRemoveMenuItem, isEditable}) => {
    ...
    return (
        <div className="border rounded p-4 shadow mb-6">
            ...
            <input className="border p-2 w-full mb-4" type="number" name={`price-${idx}`} value={item.price} onChange={(e) => onChangeMenuItem(e, idx)} placeholder="Price" readOnly={!isEditable}/>
            <div>
                {foodTags.map(tag => (
